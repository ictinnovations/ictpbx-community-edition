...
(
    usersession_id            INT                  AUTO_INCREMENT ,
...
(
   coverpage_id                 INT(11)                NOT NULL PRIMARY KEY AUTO_INCREMENT,
   title                        VARCHAR(250)           default NULL,
   description                  VARCHAR(1000)          default NULL,
   tenant_id                    INT(5)                 default NULL,                   
   created_by                   INT(5)                 default NULL
) ENGINE = InnoDB;
CREATE INDEX coverpage_tenant_id ON coverpage (tenant_id);
...
CREATE INDEX spool_account_id ON spool (account_id);
CREATE INDEX spool_provider_id ON spool (provider_id);
...
CREATE INDEX spool_result_application_id ON spool_result (application_id);
...
CREATE INDEX contact_group_tenant_id ON contact_group (tenant_id);
...
CREATE INDEX contact_link_contact_id ON contact_link (contact_id);
CREATE INDEX contact_link_group_id ON contact_link (group_id);
...
CREATE INDEX provider_tenant_id ON provider (tenant_id);
