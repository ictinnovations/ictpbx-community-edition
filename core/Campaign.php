...
class Campaign
{
  ...
  /** @var string */
  public $reason = Campaign::REASON_NORMAL;
  ...
  public static function search($aFilter = array())
  {
    ...
    $query = "SELECT c.*, p.type AS program_type,
                     (SELECT count(transmission_id) from transmission where campaign_id = c.campaign_id AND status = 'failed') AS failed,
                     (SELECT count(transmission_id) from transmission where campaign_id = c.campaign_id AND status = 'completed') AS successful
              FROM " . $from_str;
  ...
  public function delete()
  {
    ...
    $activity->userlogs("Delete Compaign ID:$this->compaign_id");
  ...
  public function save()
  {
    ...
    if (isset($data['campaign_id']) && !empty($data['campaign_id'])) {
      ...
      $activity = new Activity();
      $activity->userlogs("Update Compaign [ID: $this->compaign_id]");
    } else {
      // add new
      ...
      $activity = new Activity();
      $activity->userlogs("Create Compaign  [ID:$this->compaign_id]");
    }
    return $result;
  }
