...
class Template extends Message
{
  ...
  public static function search($aFilter = array())
  {
    ...
    $query = "SELECT template_id, tenant_id, name, subject, type, length, description FROM " . $from_str;
  ...
  protected function load()
  {
    ...
    if ($data) {
      ...
      $this->body = quoted_printable_decode($data['body']);
      //$this->body = preg_replace('/\s*style="[^"]*"/i', '', $this->body);
      $this->body = preg_replace('/\s*class="[^"]*"/i', '', $this->body);
      $this->body = preg_replace('/[^\x20-\x7E]/', ' ' , $this->body);
      $this->body_alt = quoted_printable_decode($data['body_alt']);
