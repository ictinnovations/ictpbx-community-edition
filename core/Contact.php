...
class Contact
{
  ...
  public function __construct($contact_id = NULL, $transmission = NULL)
  {
  ...
  public static function locate($contact, $contactField = 'phone')
  {
    ...
    if ($listContact) {
      ...
      $oContact = new Contact($aContact['contact_id'], 1);
      return $oContact;
    }
    return false;
  }
  ...
  public static function search($aFilter = array(), $full = false)
  {
    ...
    if ($full) {
      $query = "SELECT c.contact_id, c.tenant_id, c.first_name, c.last_name, c.phone, c.email, c.address, "
        . "c.custom1, c.custom2, c.custom3, c.description FROM " . $from_str  . " ORDER BY c.contact_id DESC " . $limitSql;
    } else {
      $query = "SELECT c.contact_id, c.tenant_id, c.first_name, c.last_name, c.phone, c.email FROM " . $from_str  . " ORDER BY c.contact_id DESC " . $limitSql;
    }
    ...
    if (empty($aContact) && isset($aFilter['contact_id']) && $aFilter['contact_id'] == Contact::COMPANY) {
      $oContact = new Contact($aFilter['contact_id'], 1);
      $singleContact['contact_id'] = $oContact->contact_id;
      $singleContact['tenant_id'] = $oContact->tenant_id;
  ...
  {
    $query = $this->transmission ? "SELECT * FROM " . self::$table . " WHERE contact_id='%contact_id%'" : $this->query();
  ...
  public function delete()
  {
    ...
    mysqli_query(DB::$link , "DELETE from contact_link where contact_id=" . $this->contact_id);
    $data = [
      'contact_id' => $this->contact_id,
      'is_deleted' => 1
    ];
    return DB::update(self::$table, $data, 'contact_id');
    //DB::delete(self::$table_link, 'contact_id', $this->contact_id);
    //$activity = new Activity();
    //$activiy->userlogs("Delete Contact Name:($this->first_name) -ID:($this->contact_id)");
  ...
  public function query()
  {
    ...
    if ($is_admin) {
      ...
      $query = "SELECT * FROM " . self::$table . " WHERE contact_id='%contact_id%' AND tenant_id=$tenant_id";
    } else {
      $query = "SELECT * FROM " . self::$table . " WHERE contact_id='%contact_id%' and created_by=$user_id";
    }
    return $query;
  }
  ...
  public function save()
  {
    ...
    if (isset($data['contact_id']) && !empty($data['contact_id'])) {
      ...
      $activity = new Activity();
      $activity->userlogs("Update Contact Name:($this->first_name) -ID:($this->contact_id)");
    } else {
      // add new
      ...
      $activity = new Activity();
      $activity->userlogs("Create New Contact Name:($this->first_name) -ID:($this->contact_id)");
    }
    return $result;
  }
  ...
  public function link_delete($group_id = null)
  {
    ...
    $result_add = mysqli_query(DB::$link, "DELETE from contact_link where contact_id=" . $this->contact_id . " AND group_id=" . $group_id);
    $result = mysqli_num_rows($get_link_count) - 1;
    //$count_contact = mysqli_query(DB::$link, "SELECT * from contact_link where group_id=".$group_id." GROUP BY contact_id");
