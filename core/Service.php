...
class Service
{
  ...
  public static function get_route() {
    ...
    $transmission = new Transmission($oSession->transmission->transmission_id);
    $transmission->status = Transmission::STATUS_FAILED_PROVIDER;
    $transmission->save();
  ...
  public function application_execute(Application $oApplication, $command = '', $command_type = 'string')
  {
    ...
    if (($oApplication->defaultSetting & Application::REQUIRE_PROVIDER) == Application::REQUIRE_PROVIDER) {
      ...
      // load provider
      $oProvider = Provider::load($oApplication->oTransmission->oSpool->provider_id);
      if($oProvider) $oToken->add('provider', $oProvider);
      else {
        // Update Database 
      };
  ...
  protected function config_save($config_type, Token $oToken, $config_name = 'default', $domain = 'default')
  {
    $template = $this->template_path($config_type);
    $aSetting = $oToken->render_template($template);
    $oGateway = $this->get_gateway();
    $oGateway->config_save($config_type, $config_name, $aSetting, $domain);
  }

  protected function config_delete($config_type, $config_name = 'default', $domain = 'default')
  {
    $oGateway = $this->get_gateway();
    $oGateway->config_delete($config_type, $config_name, $domain);
  }
}
