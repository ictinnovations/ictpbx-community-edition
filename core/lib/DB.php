...
class DB
{
  ...
  static function connect($link_new = FALSE)
  {
    ...
    $db_user = Conf::get('db:user', 'root');
    $db_pass = Conf::get('db:pass', '');
    $db_name = Conf::get('db:name', 'ictfax');
  ...
  static function update($table, &$values, $primary_key = FALSE, $check_auth = FALSE, $foreign_table = '', $foreign_key = '')
  {
    ...
    while ($column = mysqli_fetch_assoc($col_result)) {
      ...
      switch ($column_name) {
        ...
        default:
          // Check if tenant id alreay set
          if ($table != 'tenant' && $column_name === 'tenant_id' && (!array_key_exists($column_name, $values) || $values[$column_name] == NULL)) {
            $columns[$column_name] = $column;
            $columns[$column_name]['Value'] = $tenant_id;
         } else if (array_key_exists($column_name, $values) && $values[$column_name] !== NULL) {
