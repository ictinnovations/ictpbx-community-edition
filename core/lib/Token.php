...
class Token
{
  ...
  public function render_template($template, $default_value = Token::KEEP_ORIGNAL)
  {
    ...
    // If $template is an absolute path that starts with the loader base dir,
    // convert it to a relative path so Twig can resolve it correctly.
    // Twig's validateName() strips the leading '/' from absolute paths,
    // turning '/abs/path/file.json' into 'abs/path/file.json' and then
    // appending it to the loader root, producing a non-existent combined path.
    $template_name = $template;
    if (!empty($template) && $template[0] === '/' && strpos($template, $template_dir . '/') === 0) {
      $template_name = substr($template, strlen($template_dir) + 1);
    }
    ...
    /* ------------------------------------------------------ RENDER TEMPLATE */
    return $twig->render($template_name, $this->token->getDataCopy());
  }
  ...
  public function render_string_callback($org_matchs)
  {
    ...
    // } 
      ...
      return ''; // if nothing above worked then return original
    }
  }
  ...
  /**
   * Find and return all token, which exist in the given text
   * @param string $subject what we need to examine
   * @return array a list of tokens found
   */
  public static function find_token($subject) {
    $regex  = "/\[(([\w]+)(:[\w]+)*)\]/";
