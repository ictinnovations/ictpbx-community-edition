...
class Program
{
  ...
  public static function search($aFilter = array())
  {
    ...
    $query = "SELECT program_id, tenant_id, name, type, parent_id FROM " . $from_str;
  ...
  public function save()
  {
    ...
    if ($this->type === 'forward') {
      $Voice = new Voice();
      $Voice->config_update_did_assign();
    }
    return $result;
  }
  ...
  public function transmission_create($contact_id, $account_id, $direction = Transmission::OUTBOUND)
  {
    ...
    $oTransmission->tenant_id = $this->tenant_id;
  ...
  public function execute()
  {
    ...
    foreach ($listApplication as $aApplication) {
      $oApplication = Application::load($aApplication['application_id']);
      $oApplication->_prepare($this->oTransmission);
      if ($oApplication->weight == Application::ORDER_INIT) {
        $firstApplication = $oApplication;
      }
    }
    if ($firstApplication) {
     $oSession = Session::get_instance();
     $destination     = isset($oSession->transmission->destination->phone) ? $oSession->transmission->destination->phone : null;
     $isMatchFound = false;
     if ($destination) {
       $oAccount     = new Account();
       $user_account = $oAccount->search(['type' => 'did']);
       $query = "select * from account where phone = $destination and type= 'did'";
       $result = DB::query('account' , $query);
    if (mysqli_num_rows($result)) {
     $row = mysqli_fetch_assoc($result); // fetch the first row
     $account_id = $row['account_id'];
     $isMatchFound = true;
     $contact = Contact::locate($oSession->transmission->source->phone);
     if(!$contact){
     $contact = new Contact();
     $contact->phone = $oSession->transmission->source->phone;
     $contact->save();
     }
     $contact_id =  $contact->contact_id;
     $oSession->internal_created_by = $row['created_by'];
     $oSession->internal_tenant_id = $row['tenant_id'];
    }
     } // end if($destination) DID lookup
     if ($isMatchFound && $this->type != 'sendemail') {
     return $this->internal_fax($account_id , $contact_id);
     } else {
      $firstApplication->_execute($this->oTransmission);
     }
      return true;
    }
    return false;
   ...
   public function internal_fax($account_id , $contact_id){
      $this->oTransmission->status = Transmission::STATUS_COMPLETED;
      $this->oTransmission->oSpool->status = Transmission::STATUS_COMPLETED;
      $time = time();
      $spool_id = $this->oTransmission->oSpool->spool_id;
      $query = "update spool set time_connect= $time where spool_id= $spool_id";
      $result = DB::query('spool' , $query);
      $this->oTransmission->save();
      $oSession = Session::get_instance();
      $internal_fax = new InternalFax();
      //$internal_fax->account_id = $oSession->transmission->account->account_id;
      //$internal_fax->contact_id = $this->oTransmission->contact_id;
      $internal_fax->account_id = $account_id;
      $internal_fax->contact_id = $contact_id;
      $internal_fax->transmission_id = $oSession->transmission->transmission_id;
      $internal_fax->type = $this->type;
      $internal_fax->name = $this->name;
      $internal_fax->oTransmission = $this->oTransmission;
      $internal_fax->created_by = $oSession->internal_created_by;
      $internal_fax->outbound_created_by = $oSession->user->user_id;
      $internal_fax->tenant_id = $oSession->internal_tenant_id;
      $internal_fax->spool_id = $this->oTransmission->oSpool->spool_id;
      $internal_fax->execute();
  }  
  ...
  {
    $email_to_fax = $fax_to_email = false;
    ...
    if ($oAccount) {
       // ===> Check if this user have permission to reveived or send fax
       $user_permission = User::fetch_permission($oAccount->user_id);
       // Check if have Email to Fax permission
       if ($email_to_fax && strpos($user_permission, 'email_to_fax') === false) {
         throw new CoreException("404", "User don't have permission to Receive Fax through Email.");
       }
       // Check if have Fax to Email permission
       if ($fax_to_email && strpos($user_permission, 'fax_to_email') === false) {
         throw new CoreException("404", "User don't have permission to Send Fax on Email.");
       }
  ...
  public function process()
  {
    ...
    if ($this->oTransmission->oSpool->is_done()) {
      ...
      if (!empty($this->result['pages'])) {
        $this->oTransmission->pages = (int)$this->result['pages'];
      }
