...
class Application
{
  ...
  {
    global $path_root;
    global $freeswitch_log;
    global $channelID;
    $this->oTransmission = &$oTransmission;
    $Transmission = base64_encode(serialize($this->oTransmission));
    $Application_data = base64_encode(serialize($this));
    $channel = base64_encode(serialize($channelID));
    Corelog::log("Executing application : $this->type($this->application_id)", Corelog::DEBUG);
    if ($this->type == "fax_send" || $this->type == "fax_receive") {
      $daemon = $path_root . DIRECTORY_SEPARATOR . 'bin' . DIRECTORY_SEPARATOR . 'Faxcron.php';
      $command = "php $daemon $this->type $this->application_id $freeswitch_log $Transmission $Application_data $channel > /dev/null 2>&1 &";
      exec($command, $output, $result);
      if ($result === 0) {
        Corelog::log("Fax monitering started | application : $this->type($this->application_id)", Corelog::DEBUG);
      } else {
        Corelog::log("Fax monitering Crashed | application : $this->type($this->application_id)", Corelog::DEBUG);
      }
    }
