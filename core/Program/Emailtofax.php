...
class Emailtofax extends Program
{
  ...
  public function program_completed($program_type, Program &$oProgram)
  {
    ...
    if ($program_type == 'primary') {
      ...
      if ($oProgram->result['result'] == 'success') {
        ...
        $oContact = new Contact($this->oTransmission->contact_id , 1);
        
        $this->send_fax($oTemplate);
      } else {
   ...
   * **************************************************************************
   */
  public function send_fax($oTemplate)
  {
    // prepare data for new program
    $oDocument = new Document();
    $oDocument->file_name = $oTemplate->attachment;
    $oDocument->name = $oTemplate->name;
    $oDocument->save();
